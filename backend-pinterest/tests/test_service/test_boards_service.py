...
async def test_board_pin_associations(
    db_session: AsyncSession, sample_user, sample_pin
):
...
async def test_board_pin_associations_forbidden(
    db_session: AsyncSession, sample_user, another_user, sample_pin
):
    board = await create_board(
        db_session, sample_user, BoardCreate(title="Pin Board 2")
    )
