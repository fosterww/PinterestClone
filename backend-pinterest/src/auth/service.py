...
class AuthService:
    ...
    async def register_user(self, data: UserCreate) -> UserModel:
        try:
            ...
            raise AppError(detail="Failed to register user")

    async def authenticate_user(self, username: str, password: str) -> UserModel | None:
        try:
            user = await self.user_repo.get_user_by_username_or_email(
                username, username
            )
            ...
            raise AppError(detail="Failed to authenticate user")
        ...
        try:
            ...
            if user:
                if not user.google_id:
                    user = await self.user_repo.link_google_id(
            ...
            await self.db.rollback()
            logger.exception("Error getting or creating Google user")
            raise AppError(detail="Failed to authenticate Google user")
    ...
    async def create_user_session_and_tokens(self, user: UserModel) -> dict:
        try:
            ...
            await self.db.rollback()
            logger.exception("Error creating user session and tokens")
            raise AppError(detail="Failed to create user session")
    ...
    async def refresh_user_token(self, refresh_token: str) -> dict:
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Session validation failed for session {token.session_id}"
                )
            ...
            await self.db.rollback()
            logger.exception("Error refreshing user token")
            raise AppError(detail="Failed to refresh token")
    ...
    async def logout_user(self, token: str):
        try:
            ...
            raise AppError(detail="Failed to logout user")
